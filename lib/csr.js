...
const { BitString, Integer, ObjectIdentifier, Sequence, Set, Constructed, Utf8String } = asn1
...
const createCRI = (orgName, commonName, pubKey) => new Sequence({ value: [
  ...
  new Sequence({ value: [
    ...
    new BitString({ valueHex: ArrayBufferFrom(Buffer.concat([Buffer.from([0x04]), pubKey])) })
  ] }),
  new Constructed({
...
const createCSR = (cri, signature) => new Sequence({ value: [
  cri,
  new Sequence({ value: [
    new ObjectIdentifier({ value: '1.2.840.10045.4.3.2' }) // 0x2a 0x86 0x48 0xce 0x3d 0x04 0x03 0x02
  ] }),
