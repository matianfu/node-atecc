  ...
  */
  async presetAsync () {
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      // TODO exec must throw error for ....
  ...
  async generateCsrAsync (opts) {
    ...
    let { o, cn, slot, check } = opts
    if (typeof o !== 'string' || !o.length || o.length > 64) { throw new Error('bad o name') }
    if (typeof cn !== 'string' || !cn.length || cn.length > 64) { throw new Error('bad cn name') }
    ...
    // TODO
    if (!verified.equals(Buffer.alloc(1))) throw new Error('verify failed')
