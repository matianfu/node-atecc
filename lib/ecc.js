...
*/
  ...
  */
  ...
  async atcertSetCertElement (cert_def, cert_loc, cert, cert_size, data, data_size) {
    let eff_offset = 0
    if (cert_loc.count === 0) return
  ...
  atcacertGetTbs (cert_def, cert, cert_size) {
    let eff_offset = get_effective_offset(cert_def, cert, cert_def-> tsb_cert_loc.offset + cert_def.tas_cert_loc.count)
  ...
  async atcacertCreateCsrAsync (csr_def) {
    ...
    let tbs_digest = Buffer.alloc(ATCA_BLOCK_SIZE)
    ...
    atcacertGetTbsDigest(csr_def, csr,  
