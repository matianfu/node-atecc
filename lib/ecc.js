...
*/
    ...
    for (let i = 0; i < 20; i++) {
      try {
        ...
        if (data.toString('hex') === '04113343') {
          console.log(`wake read ${i === 0 ? 'once' : i === 1 ? 'twice' : i + 1 + ' times'}`)
          return
        } else {
          console.log(`wake read unexpected data: ${data.toString('hex')}`)
        }
      } finally {
        await Promise.delay(128)
      }
    }
  ...
  // hal
  async atsendAsync (packet) {
    let head = Buffer.alloc(6)   
    head[0] = 0x03  // Word Address Value command, section 6.2.1
    head[1] = packet.txsize
    head[2] = packet.opcode
    head[3] = packet.param1
    head[4] = packet.param2 >> 8
    head[5] = packet.param2

    let tail = packet.data.slice(0, packet.txsize - 5)
    let buf = Buffer.concat([head, tail])

    debug('atsend', buf.toString('hex'))
   
    await this.writeAsync(buf) 
  }

  async atreceiveAsync (len) {
    let retries = 0

    for (let i = 0; i < 100; i++) {
      try {
        let data = await this.readAsync(len) 
        return data
      } catch (e) {
  ...
  atCalcCrc (packet) {
    ...
    head[0] = packet.txsize
    head[1] = packet.opcode
    head[2] = packet.param1
    head[3] = packet.param2 >> 8
    head[4] = packet.param2

    let payload = packet.txsize - 5 - ATCA_CRC_SIZE
    let buffer = Buffer.concat([head, packet.data.slice(0, payload)])
    let crc = this.atCrc(buffer)

    let idx = packet.txsize - 5 - ATCA_CRC_SIZE

    packet.data[idx] = crc[0]
    packet.data[idx + 1] = crc[1]
  }


  // lib/atca_command.c +499
  // this function complete a read packet
  atRead (packet) {
    ...
    this.atCalcCrc(packet)
  }

  // lib/basic/atca_basic.c +392
  // this command do wake-idle cycle
  async atcabExecuteCommandAsync (packet) {
    // TODO atGetExecTime 

    await this.wakeAsync()
    await this.atsendAsync(packet)

    // TODO calculate time
    await Promise.delay(100)

    let data = await this.atreceiveAsync(packet.rxsize)

    debug('received data', data)

    

    let crc = this.atCrc(data.slice(0, -2))
  ...
  // lib/basic/atca_basic.c +452
  atcabGetAddr (zone, slot, block, offset) {
    let addr
    let mem_zone = zone & 0x03
    ...
    console.log('zone', zone, 'slot', slot, 'block', block, 'offset', offset, 'addr', addr)
  ...
  async atcabReadZoneAsync (zone, slot, block, offset, len) {
    ...
    let addr = this.atcabGetAddr(zone, slot, block, offset) 
    ...
    let packet = {
      _reserved: null,
      txsize: null,     // uint8_t      <- start
      opcode: null,     // uint8_t
      param1: zone,     // uint8_t
      param2: addrLE,   // uint16_t
      data: Buffer.alloc(130),    //    <- end
      execTime: null,
      rxsize: null
    }
    ...
    return await this.atcabExecuteCommandAsync(packet)
  }
  ...
  async atcabReadBytesZoneAsync (zone, slot, offset, length) {
    ...
    if (zone !== ATCA_ZONE_CONFIG && zone !== ATCA_ZONE_OTP && zone !== ATCA_ZONE_DATA)
      throw new Error('ATCA_BAD_PARAM')
