...
*/
  ...
  */
  ...
  // return random, also update seed
  async atcabRandomAsync () {
    let packet = {
      txsize: RANDOM_COUNT,
      opcode: ATCA_RANDOM,
    ...
    let rand = await this.atcaExecuteCommandAsync(packet)

    debug('atca random', rand)

    if (rand.length !== RANDOM_RSP_SIZE - 3) 
      throw new Error('failed')

    return rand
  }

  async atcabNonceBaseAsync (mode, zero, buf) {
    let nonce_mode = mode & NONCE_MODE_MASK
    let data, txsize

    if (nonce_mode === NONCE_MODE_SEED_UPDATE || 
        nonce_mode === NONCE_MODE_NO_SEED_UPDATE) {
      data = buf.slice(0, NONCE_NUMIN_SIZE)   
      txsize = NONDE_COUNT_SHORT
    ...
    console.log('nonce packet', packet)

    let rsp = await this.atcaExecuteCommandAsync(packet)
    if (rsp) return rsp
  }

  // load 
  async atcabNonceLoadAsync (target, buf) {
    debug('nonce load', target, buf.length, buf)
    ...
    return this.atcabNonceBaseAsync(mode, 0, buf)
  }

/**
  async atcabInfoAsync () {
    let packet = {
      txsize: INFO_COUNT,
      opcode: ATCE_INFO, 
      param1:  
...
*/
  ...
  // return signature
  async atcabSignBaseAsync (mode, keyId) {
    let packet = {
      txsize: SIGN_COUNT,
      opcode: ATCA_SIGN,
      param1: mode,
  ...
  // return signature 
  // R and S in big-endian format. 64 bytes for P256 curve
  async atcabSignAsync (keyId, msg) {
    debug('sign', keyId, msg)

    // update rand
    // await this.atcabRandomAsync()

    // TODO for atecc608a this is different !!!
/**
    let nonce_target = NONCE_MODE_TARGET_TEMPKEY
    let sign_source = SIGN_MODE_SOURCE_TEMPKEY
*/

    let nonce_target = NONCE_MODE_TARGET_MSGDIGBUF
    let sign_source = SIGN_MODE_SOURCE_MSGDIGBUF

    console.log('sign msg', msg.length, msg)

    // await this.atcabNonceLoadAsync(nonce_target, msg)
    console.log('>>>> nonce >>>>')
    await this.nonceAsync(msg)
    console.log('>>>> nonce >>>>')

    console.log(await this.stateInfoAsync())

    let sig = await this.atcabSignBaseAsync(SIGN_MODE_EXTERNAL | sign_source, keyId)

    console.log('sig length', sig.length)
    console.log(sig.slice(0, 32))
    console.log(sig.slice(32, 64))
    console.log(sig.slice(64))
  ...
  /**
  static int get_effective_offset(const atcacert_def_t* cert_def, 
                                  const uint8_t* cert, 
                                  size_t ref_offset)

  return 0 for non-x509 or non-SNSRC_STORED_DYNAMIC
  */
  getEffectiveOffset (cert_def, cert, ref_offset) {
    if (cert_def.type !== CERTTYPE_X509) return 0
    if (cert_def.sn_source !== SNSRC_STORED_DYNAMIC) return 0

    let sn_offset = cert_def.std_cert_elements[STDCERT_CERT_SN].offset
  ...
  atcacertGetTBS (cert_def, cert, cert_size) {
    ...
    if (offset + count + eff_offset > cert_size)
      throw new Error('bad cert')

    let tbsStart = offset
    let tbsEnd = offset + count + eff_offset

    console.log('tbs start (offset), end, count, eff_offset', tbsStart, tbsEnd, count, eff_offset)
    let tbs = cert.slice(tbsStart, tbsEnd)
    console.log(tbs.length, tbs)
  ...
  atcacertSetSignature (cert_def, cert, cert_size, signature) {

    console.log('signature', signature)

    debug('atcacertSetSignature', cert_def, cert, cert_size, signature)

    // for device 2 offset 180 count 74 
    let sig_offset = cert_def.std_cert_elements[STDCERT_SIGNATURE].offset
    ...
    if (sig_offset >= cert_size) throw new Error('out of bounds')

    let oldSigSize = cert_size - sig_offset
    // dsig is a buffer
//    let dsig = DER.encodeEcdsaSignature(signature)
    ...
    // cert length is located at cert[1]
    // oldLen is an object
    let oldLen = DER.decodeLength(cert, 1)

    // newLen is a buffer
    let newLenValue = oldLen.value + dsig.length - (cert_size - sig_offset)
    console.log('new len value', newLenValue)
    let newLen = DER.encodeLength(newLenValue)

/**
    console.log(DER.encodeLength(251))
    console.log(DER.encodeLength(252))
    console.log(DER.encodeLength(253))
    console.log(DER.encodeLength(254))
    console.log(DER.encodeLength(255))
    console.log(DER.encodeLength(256))
    console.log(DER.encodeLength(257))
    console.log(DER.encodeLength(258))

    console.log(DER.decodeLength(Buffer.from([0x82, 0x01, 0x00])))
    console.log(DER.decodeLength(Buffer.from([0x82, 0x01, 0xB1])))
*/
/**
    if (newLen.length !== oldLen.data.length) {
      console.log('newLen.length', newLen.length, newLen)
      console.log('oldLen', oldLen.data, oldLen.value)
      throw new Error('bad cert template')
    }
*/
    ...
    return Buffer.concat([
      cert.slice(0, 1), // tag
      newLen,
      cert.slice(1 + oldLen.data.length, sig_offset),
      dsig
    ])
  }
  ...
  async atcacertCreateCsrAsync (csr_def) {
    ...
    let private_key_slot = csr_def.private_key_slot

    if (pub_dev_loc.is_genkey) {
      console.log('gen pubkey from key slot', key_slot)
      pub_key = await this.atcabGenPubKeyAsync(key_slot)
    } else {
      console.log('read pubkey from key slot', key_slot)
      pub_key = await this.atcabReadPubKeyAsync(key_slot)
    }

    // insert public key into csr template 
    this.atcertSetCertElement(csr_def, pub_loc, csr, csr_size, pub_key)

    tbs_digest = this.atcacertGetTBSDigest(csr_def, csr, csr_size)

    console.log('tbs_digest', tbs_digest)

    sig = await this.atcabSignAsync(private_key_slot, tbs_digest)

    console.log('signature', sig.length, sig)

    return this.atcacertSetSignature(csr_def, csr, csr_size, sig)
  }

  async awsGenCsrAsync () {
    return this.atcacertCreateCsrAsync(g_csr_def_2_device)
  }
