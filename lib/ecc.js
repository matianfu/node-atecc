...
*/
  ...
  async atcabWriteAsync (zone, addr, _data, mac) {
    ...
    await this.atcabExecuteCommandAsync(packet)
  }
  ...
  async atcabWriteZoneAsync (zone, slot, block, offset, data) {
    ...
    console.log(zone, slot, block, offset, data)
    ...
    this.atcabWriteAsync(zone, addr, data)
  }
  ...
  async writeAWSConfigAsync () {
    ...
    console.log('writing aws config')

    this.atcabWriteConfigZoneAsync(Buffer.from(AWS_CONFIG))
  }
  ...
  async atcabLockAsync(mode, summary_crc) {
    ...
    this.atcabExecuteCommandAsync(packet)
  }

  async atcabLockConfigZoneAsync () {
    this.atcabLockAsync(LOCK_ZONE_NO_CRC | LOCK_ZONE_CONFIG, 0)
  }

  async atcabLockConfigZoneCrcAsync (summary_crc) {
    this.atcabLockAsync(LOCK_ZONE_CONFIG, summary_crc)
  }

  async atcabLockDataZoneAsync () {
    this.atcabLockAsync(LOCK_ZONE_NO_CRC | LOCK_ZONE_DATA, 0)
  }

  async atcabLockDataZoneCrcAsync (summary_crc) {
    this.atcabLockAsync(LOCK_ZONE_DATA, summary_crc)
  }

  async atcabLockDataSlot (slot) {
    this.atcabLock(slot << 2 | LOCK_ZONE_DATA_SLOT, 0)
  }
