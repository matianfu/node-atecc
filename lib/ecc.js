...
*/
  ...
  async atcabExecuteCommandAsync (packet) {
    ...
    let payload = Buffer.concat([
      Buffer.from([txsize, opcode, param1, param2, param2 >> 8]), 
      data
    ])

    let crc = CRC(payload)
    await this.writeAsync(Buffer.concat([
      Buffer.from([0x03]),
      payload,
      Buffer.from([crc, crc >> 8])
    ]))

    // TODO calculate time
    await Promise.delay(100)

    // data = await this.atreceiveAsync(rxsize)
    data = await this.readAsync(rxsize)
    debug('atreceive', data)

    // TODO
    if (CRC(data.slice(0, -2)) !== data.slice(data.length - 2).readUInt16LE(0))
  ...
  // lib/basic/atca_basic???
  async atcabWriteAsync (zone, address, data, mac) {
    /**
    let packet = {
      packet.param1 = zone
      ...
      if (zone & ATCA_ZONE_READWRITE_32) {
        if (mac) {
          packet.data = Buffer.concat(data, mac)
        } else {
          packet.data = data
        }
      } else {
...
*/
  ...
  async atcabWriteZoneAsync (zone, slot, block, offset, data) {
    if (data.length !== 4 || data.length !== 32) throw new Error('BAD_PARAM')

    let addr = atcabGetAddr(zone, slot, block, offset)

    if (data.length === ATCA_BLOCK_SIZE) zone = zone | ATCA_ZONE_READWRITE_32

    this.atcabWrite(zone, addr, data, null)
  }

  async atcabWriteBytesZone (zone, slot, offset_bytes, data, length) {
    if (zone !== ATCA_ZONE_CONFIG && zone !== ATCA_ZONE_OTP && zone !== ATCA_ZONE_DATA) {
      throw new Error('BAD_PARAM')
    } else if (zone === ZTCA_ZONE_DATA && slot > 15) {
      throw new Error('BAD_PARAM')
    } else if (length === 0) {
      return
    }
    ...
    while (data_idx < length) {
      if (cur_word === 0 && length - data_idex >= ATCA_BLOCK_SIZE && !(zone === ATCA_ZONE_CONFIG && cur_block === 2)) {
        // await atcabWriteZone(zone, slot, cur_block, 0
      }
    }
  }

  async atcabWriteConfigZone (data) {
    // TODO validate

    let config_size = this.atcabGetZoneSize(ATCA_ZONE_CONFIG, 0)
    // atcabWriteBytesZone(ATCA_ZONE_CONFIG, 0, 16,
  }

  async atcabWriteConfigZoneAsync () {
